...
exports.villagerStatusEmail = villagerStatusEmail;
exports.escalationEmail = escalationEmail;
...
function wrap(title, body, locale) {
    const d = dict(locale);
    return `
...
function villagerStatusEmail(args) {
    const body = `
...
function escalationEmail(args) {
    const roleLabel = args.toRole === "pdo" ? "PDO" : args.toRole === "tdo" ? "TDO" : "DDO";
    const reasonText = args.locale === "kn"
        ? `ಕಾರಣ: ${args.reason}`
        : args.locale === "hi"
            ? `कारण: ${args.reason}`
            : `Reason: ${args.reason}`;
